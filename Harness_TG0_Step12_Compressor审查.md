# Harness Engine — TG0 Step 12 @harness/compressor Adversarial Review
...
| 4 | 5 | @harness/adapters | **A-** | 完整 650 行 |
| 5 | 6 | config.ts | **B+** | NaN 全覆盖 |
| 6 | 11 | @harness/memory | **B+** | TTL/logic 正确 |
| 7 | 4 | @harness/tools | **B+** | 11 条修复稳固 |
| 8 | 10 | @harness/subagent | **B** | 3 致命已修 |
| **9** | **12** | **@harness/compressor** | **C+** | 管道逻辑正确、trigger/target/cache 机制清晰。但 **F-1 是数据破坏性 bug**（静默删除工具消息）+ **F-2 是系统性低估**（整个管道基于不准确的 token 数运行）。修完可升至 B |
