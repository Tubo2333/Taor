...
任何一个外部 TS 文件写 `import { HarnessConfig } from "@harness/engine"` 然后尝试构造 config 对象，IDE 会在这些类型上报 `Cannot find name`。
...
`defineTool()` 的 Zod 重载必须将 `z.ZodType` 转换为 `JSONSchema`。这需要 `zod-to-json-schema` 或手写转换器。§十三的包结构中完全没有提到这个依赖。它是 `@harness/tools` 的 dependency？peer dependency？bundled？不声明的话，用户 `npm install @harness/tools` 后第一个 `defineTool(z.object({...}))` 调用就会在运行时炸。
