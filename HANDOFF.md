...
开源的 **Taorering 框架** — TypeScript AI agent 运行时，基于 Claude Code 泄露源码设计。
...
6   ✅ @taor/core/config.ts                           [配置校验，5 NaN guards]
7   ✅ @taor/core/harness.ts (TAOR 循环)              [核心引擎，~1080行]
...
### Step 7：@taor/core/harness.ts（TAOR 核心引擎，~1080 行）

**架构决策：依赖反转** — `@taor/core` 不能运行时 import `@taor/tools`/`@taor/adapters`（circular project references）。Harness 构造函数接收预实例化的 `IAdapter` + `IToolRegistry`（私有结构接口），由 `createHarness()` 在 `@taor/engine` 组装。
...
`@taor/core` 使用私有结构接口 → `@taor/engine` 用 `as any` 桥接到真实类型。**变更任一 canonical 接口时必须同步更新对应 structural 接口。**
...
## 九、下一步：TG0 Step 8 — @taor/permission
